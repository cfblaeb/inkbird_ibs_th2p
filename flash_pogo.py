...
PHY_SRAM2_ADDR = 0x20000000
PHY_SRAM2_END = 0x20010000

USBDEVFS_RESET = 0x5514

__progname__ = 'PHY62x2/ST17H66B/TG7100B Utility'
__filename__ = 'flash_pogo.py'
__version__ = "08.02.25"

def find_usb_device_for_port(port):
	port_name = os.path.basename(port)
	sysfs_path = '/sys/class/tty/%s' % port_name
	...
	try:
		...
		for i in range(len(parts), 0, -1):
			...
			busnum_path = os.path.join(candidate, 'busnum')
			devnum_path = os.path.join(candidate, 'devnum')
			if os.path.isfile(busnum_path) and os.path.isfile(devnum_path):
				with open(busnum_path) as f:
					busnum = int(f.read().strip())
				with open(devnum_path) as f:
					devnum = int(f.read().strip())
				usb_dev = '/dev/bus/usb/%03d/%03d' % (busnum, devnum)
...
def reset_usb_device(port):
	usb_dev = find_usb_device_for_port(port)
	if usb_dev == None:
		print("Warning: USB device for '%s' not found, skipping USB reset." % port)
		return False
	try:
		print('Reset USB device: %s' % usb_dev)
		fd = os.open(usb_dev, os.O_WRONLY)
		fcntl.ioctl(fd, USBDEVFS_RESET, 0)
		os.close(fd)
	except Exception as e:
		print("Warning: USB reset failed for '%s': %s" % (usb_dev, e))
		return False
	if not wait_for_port(port):
		print("Warning: Port '%s' did not reappear after USB reset." % port)
		return False
	return True
...
def IsSramAddr(addr):
	if (addr & PHY_SRAM_ADDR) == PHY_SRAM_ADDR:
		return True
	# Newer stage1 layouts place .data/.bss in SRAM2 at 0x20000000.
	return addr >= PHY_SRAM2_ADDR and addr < PHY_SRAM2_END
